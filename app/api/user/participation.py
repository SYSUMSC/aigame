...
async def get_participation(user_id:int, session: AsyncSession = Depends(get_session)):
    try:
        # 获取用户参赛信息
        statement = select(Participation).where(Participation.user_id == user_id)
...
async def quit_competition(user_id: int, competition_id: int, session: AsyncSession = Depends(get_session)):
    #退出比赛
    try:
        # 检查用户是否已参加比赛
        existing_participation = await session.execute(select(Participation).where(Participation.user_id == user_id, Participation.competition_id == competition_id))
        participation_db = existing_participation.scalar_one_or_none()
        if not participation_db:
            return ResponseModel(code=1, msg="用户未参加比赛")
        
        # 删除参赛记录
        session.delete(participation_db)
        await session.commit()
